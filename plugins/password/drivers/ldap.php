...
function password_save($curpass, $passwd)
{
    ...
    // Building user DN
    $userDN = str_replace('%login', $_SESSION['username'], $cmail->config->get('password_ldap_userDN_mask'));
    ...
    // Connection Method
    switch($cmail->config->get('password_ldap_method')) {
        case 'user': $binddn = $userDN; $bindpw = $curpass; break;
        case 'admin': $binddn = $cmail->config->get('password_ldap_adminDN'); $bindpw = $cmail->config->get('password_ldap_adminPW'); break;
    ...
    $ldapConfig = array (
        'binddn'    => $binddn,
        'bindpw'    => $bindpw,
        'basedn'    => $cmail->config->get('password_ldap_basedn'),
        'host'      => $cmail->config->get('password_ldap_host'),
        'port'      => $cmail->config->get('password_ldap_port'),
        'starttls'  => $cmail->config->get('password_ldap_starttls'),
        'version'   => $cmail->config->get('password_ldap_version'),
    );
    ...
    $newCryptedPassword = hashPassword($passwd, $cmail->config->get('password_ldap_encodage'));
    ...
    if (!$userEntry->replace(array($cmail->config->get('password_ldap_pwattr') => $newCryptedPassword),$cmail->config->get('password_ldap_force_replace'))) {return PASSWORD_CONNECT_ERROR;}
