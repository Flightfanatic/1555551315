...
<p>Crystal may need to write/save files into these directories</p>
<?php
...
if (isset($_POST['sendmail']) && !empty($_POST['_from']) && !empty($_POST['_to'])) {
  ...
  if (preg_match('/^' . $RCI->email_pattern . '$/i', trim($_POST['_from'])) &&
    ...
    $headers = array(
      ...
      'To'      => trim($_POST['_to']),
      'Subject' => 'Test message from Crystal',
    );

    $body = 'This is a test to confirm that Crystal can send email.';
    $smtp_response = array();
...
<a href="../admin/index.php" class="style3"><span class="style4">Click Here to 
Go to the Admin Interface</span></a></span> </h3>
