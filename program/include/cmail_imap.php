...
class cmail_imap
{
  ...
  function connect($host, $user, $pass, $port=143, $use_ssl=null)
    ...
    $attempt = 0;
    do {
      $data = cmail::get_instance()->plugins->exec_hook('imap_connect', array('host' => $host, 'user' => $user, 'attempt' => ++$attempt));
      if (!empty($data['pass']))
        $pass = $data['pass'];
  ...
  private function _list_mailboxes($root='', $filter='*')
    ...
    $data = cmail::get_instance()->plugins->exec_hook('list_mailboxes',array('root'=>$root,'filter'=>$filter));
...
//    $this->update_cache('messagecount', $a_mailbox_cache);
  ...
  function move_message($uids, $to_mbox, $from_mbox='')
    ...
    // moving failed
    else if (cmail::get_instance()->config->get('delete_always', false)) {
