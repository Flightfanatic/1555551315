...
class cmail_plugin_api
{
   ...
   *
   * This has to be done after cmail::load_gui() or cmail::init_json()
  ...
  public function init()
  {
    ...
    // (should actually be done in cmail::imap_init() but plugins are not initialized then)
    if ($cmail->imap) {
      $hook = $this->exec_hook('imap_init', array('fetch_headers' => $cmail->imap->fetch_add_headers));
      if ($hook['fetch_headers'])
        $cmail->imap->fetch_add_headers = $hook['fetch_headers'];
    }
  }
