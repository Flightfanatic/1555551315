...
class rcube_user
{
  ...
  static function create($user, $host)
  {
    ...
    if ($user_id = $dbh->insert_id('users'))
    {
      ...
      foreach ($email_list as $row) {
        ...
        $plugin = $cmail->plugins->exec_hook('create_identity', array(
          'login' => true,
          'record' => array(
            'user_id' => $user_id,
  ...
  static function user2email($user, $first=true, $extended=false)
  {
    ...
    // SQL lookup
    if ($virtuser_query = $cmail->config->get('virtuser_query')) {
  ...
  private static function findinvirtual($pattern)
  {
    ...
    if ($virtuser_file = cmail::get_instance()->config->get('virtuser_file'))
