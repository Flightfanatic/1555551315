...
/**
 * Crystal splitter GUI class
 *
 * @constructor
 */
function cmail_splitter(attrib)
  {
  this.p1id = attrib.p1;
  this.p2id = attrib.p2;
  ...
  this.init = function()
    ...
    if (this.horizontal)
      ...
      this.layer = new cmail_layer(this.id, {x: 0, y: top, height: 10, 
    	    width: '100%', vis: 1, parent: this.p1.parentNode});
      }
    else
      {
      var left = this.p1pos.left + this.p1.offsetWidth;
      this.layer = new cmail_layer(this.id, {x: left, y: 0, width: 10, 
    ...
    cmail_event.add_listener({element: this.elm, event:'mousedown', object:this, method:'onDragStart'});
    if (bw.ie)
      cmail_event.add_listener({element: window, event:'resize', object:this, method:'onResize'});
  ...
  this.onDragStart = function(e)
    ...
    cmail_event.add_listener({element:document, event:'mousemove', object:this, method:'onDrag'});
    cmail_event.add_listener({element:document, event:'mouseup', object:this, method:'onDragStop'});
    ...
    for (var n in iframes)
      ...
      if (iframedoc)
        ...
        cmail_event.add_listener({element:iframedoc, event:'mouseup', object:this, method:'onDragStop'});
        }
      }
    }
  ...
  this.onDragStop = function(e)
    ...
    cmail_event.remove_listener({element:document, event:'mousemove', object:this, method:'onDrag'});
    cmail_event.remove_listener({element:document, event:'mouseup', object:this, method:'onDragStop'});
    this.drag_active = false;
    ...
    for (var n in iframes)
      ...
      if (iframedoc)
        ...
        cmail_event.remove_listener({element:iframedoc, event:'mouseup', object:this, method:'onDragStop'});
        }
      }

    this.set_cookie();

    return bw.safari ? true : cmail_event.cancel(e);
    };
